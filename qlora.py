...
def get_lora_model(model_name, rank=4, alpha=16, lora_dropout=0.1, bias='none'):
    ...
    if model_name == 'mistralai/Mistral-7B-v0.1' or model_name == 'meta-llama/Llama-2-7b-hf':
...
def get_unlearn_dataset_and_collator(data_path, tokenizer, add_prefix_space=True, max_length=1024, truncation=True):
...
def main(args):
    if 'opt-1.3b' in args.model_name.lower():
        model_name = 'opt-1.3b'
    elif 'llama-2-7b' in args.model_name.lower():
    ...
    trainer = SFTTrainer(
        ...
        tokenizer=tokenizer,
        train_dataset=dataset['train_retain'], # concatenate_datasets([dataset['train_retain'], dataset['train_forget']]),
