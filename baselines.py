...
import evaluate
from peft import get_peft_model, PeftConfig, PeftModel, prepare_model_for_kbit_training, PrefixTuningConfig, TaskType
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        choices={"gradient_ascent", "random_label", "gradient_ascent_kl", "gradient_ascent_descent", "negation_lora", "pi_prefix"},
    ...
    parser.add_argument(
        "--forget_size",
        type=int,
        default=100,  # or whatever default you prefer
        help="Number of training examples to forget during unlearning",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Length of the learnable prefix (k)",
    )
...
def get_gradient_ascent_plus_kl_trainer():
    class GradientAscentPlusKL(Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            # standard forward when not unlearning
            if "is_forget" not in inputs or "index" not in inputs:
                outputs = model(**inputs)
                loss = outputs.loss if hasattr(outputs, "loss") else outputs[0]
                return (loss, outputs) if return_outputs else loss

            # 1) peel off control cols
            is_forget = inputs.pop("is_forget").to(model.device)
            sample_indices = inputs.pop("index").to(model.device)

            # 2) move everything else to model.device
            ...
            # 4) shift for next-token CE
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = inputs["labels"][..., 1:].contiguous()

            # 5) compute GA loss on forget set
            ce = torch.nn.CrossEntropyLoss()
            fgt_mask = is_forget > 0
            fgt_logits = shift_logits[fgt_mask]
            fgt_labels = shift_labels[fgt_mask]
            fgt_loss = ce(fgt_logits.view(-1, model.config.vocab_size),
                          fgt_labels.view(-1)) if fgt_logits.numel() else 0.0
            fgt_loss = -fgt_loss  # gradient ascent

            # 6) fetch & stack original logits on-device
            prev_logits = torch.stack([
                torch.as_tensor(self.original_logits[idx.item()], device=model.device)
                for idx in sample_indices
            ], dim=0)

            # 7) compute KL on retain set
            rtn_mask = is_forget < 1
            # mask out -100 positions
            label_mask = (inputs["labels"] != -100)[..., 1:].to(model.device)
            rtn_prev = prev_logits[rtn_mask]
            # select corresponding current logits
            rtn_logits_all = shift_logits[label_mask & rtn_mask.unsqueeze(-1)]
            # reshape to [num_retain_tokens, vocab]
            rtn_logits = rtn_logits_all.view(-1, model.config.vocab_size)

            if rtn_prev.numel():
                kl = torch.nn.KLDivLoss(reduction="batchmean")
                rtn_loss = kl(
                    torch.log_softmax(rtn_logits, dim=-1),
                    torch.softmax(rtn_prev, dim=-1),
                )
            else:
                rtn_loss = 0.0

            loss = fgt_loss + rtn_loss
            return (loss, outputs) if return_outputs else loss
...
def main(args):
    ...
    if args.unlearn_method in ['gradient_ascent_kl']:
        ...
        if not os.path.exists(args.logits_path):
            ...
            new_original_logits = {idx: tensor.numpy() for idx, tensor in original_logits.items()}
    ...
    if args.unlearn_method.lower() == "gradient_ascent":
    ...
    elif args.unlearn_method == "negation_lora":
        ...
        base_repo = PeftConfig.from_pretrained(args.model_checkpoints).base_model_name_or_path
        ...
        assert trainable, "No trainable parameters! Check your LoRA config."

        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=dataset["train_forget"],
            eval_dataset={"forget": dataset["train_forget"]},
            tokenizer=tokenizer,
            data_collator=collator,
            compute_metrics=compute_metrics,
        )
        trainer.train()
        negate_lora_adapters(model)
        trainer.save_model(args.output_path)

    elif args.unlearn_method == "pi_prefix":
        prefix_length = 20

        TrainerClass = get_pi_prefix_trainer(
